...
async function main() {
    ...
    // Removing 'initials' as it is not in schema based on previous error log (schema says avatar currently?)
    // Schema: avatar String?
    // Let's verify schema content first
    ...
    const demoTech = await prisma.technician.upsert({
        ...
        create: {
            ...
            startDate: new Date(),
            isActive: true,
            currentStreak: 0 // Explicitly set default to be safe
        }
    });
